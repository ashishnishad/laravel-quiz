...
@section('content')
            ...
            <div class="col-xl-12 order-xl-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="post" action="{{ route('question.update',$question) }}" autocomplete='off' enctype="multipart/form-data">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
                                    ...
                                    <textarea name="question" id="question" class="form-control form-control-alternative{{ $errors->has('question') ? ' is-invalid' : '' }}" placeholder="{{ __('Question') }}" autofocus>{{ old('question',$question->question) }}</textarea>
                                 ...
                                 <div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="status" id="status" class="form-control form-control-alternative{{ $errors->has('status') ? ' is-invalid' : '' }}">
                                        <option value="enabled" {{ $question->status =="enabled" ? 'selected':'' }}>Enable</option>
                                        <option value="disabled" {{ $question->status =="disabled" ? 'selected':'' }}>Disable</option>
                                ...
                                <div class="form-group{{ $errors->has('complexity') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="complexity" id="complexity" class="form-control form-control-alternative{{ $errors->has('complexity') ? ' is-invalid' : '' }}">
                                        <option value="easy" {{ $question->complexity =="easy" ? 'selected':'' }}>Easy</option>
                                        <option value="medium" {{ $question->complexity =="medium" ? 'selected':'' }}>Medium</option>
                                        <option value="hard" {{ $question->complexity =="hard" ? 'selected':'' }}>Hard</option>
                                    </select>                                    @include('alerts.feedback', ['field' => 'complexity'])
                                </div>
                                <div class="text-center">
                                    <button type="submit" class="btn btn-success mt-4">{{ __('Update') }}</button>
                                    <a href="javascript:;" data-href="{{ route('question.remove',$question->id) }}" data-toggle="modal" data-target="#confirm-delete" class="delete btn btn-danger mt-4">Delete</a>
                                </div>
                            </div>
                        </form>
                    </div>
    ...
    @include('layouts.admin.includes.common_models', ['redirect'=>route('question.index')])
@endsection
@push('js')
